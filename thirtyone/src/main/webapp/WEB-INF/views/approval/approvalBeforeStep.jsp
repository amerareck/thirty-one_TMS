			...
			<c:if test="${not empty aprList}">
		    	<c:forEach items="${aprList}" var="apr" varStatus="i" >
					<tr>
						...
						<td class="text-center align-middle table-font-size">${apr.reviewingApprover}&nbsp;${apr.reviewingApproverPosition}</td>
			...
			<nav class="d-flex justify-content-center" style="width: 95%">
				<ul class="pagination pagination-not-effect justify-content-center pagination-size">
					<li class="page-item ${pager.startPageNo == 1 ? 'disabled' : ''}">
						<a class="page-link page-border-none text-dark" href="before?pageNo=${pager.endPageNo-pager.pagesPerGroup-1}&search=${paramObj.search}&keyword=${paramObj.keyword}" tabindex="-1" aria-disabled="true"><i class="fa-solid fa-chevron-left"></i></a>
					</li>
					<c:forEach begin="${pager.startPageNo}" end="${pager.endPageNo}" var="i">
						<li class="page-item ${pager.pageNo == i ? 'disabled' : ''}"><a class="page-link text-dark page-border-none ${pager.pageNo==i ? 'fw-bold' : ''} ${i == pager.startPageNo ? 'ms-5' : 'ms-1'} ${i == pager.endPageNo ? 'me-5' : ''}" href="${pageContext.request.contextPath}/approval/before?pageNo=${i}&search=${paramObj.search}&keyword=${paramObj.keyword}">${i}</a></li>
					</c:forEach>
					<li class="page-item ${pager.totalPageNo == pager.endPageNo ? 'disabled' : ''}">
						<a class="page-link page-border-none text-dark" href="before?pageNo=${pager.endPageNo+1}&search=${paramObj.search}&keyword=${paramObj.keyword}"><i class="fa-solid fa-chevron-right"></i></a>
					</li>
				</ul>
			</nav>
		</c:if>
	...
	<c:if test="${empty search}">
	    <c:if test="${pager.totalRows > 0}">
			<nav class="d-flex justify-content-center" style="width: 95%">
				<ul class="pagination pagination-not-effect justify-content-center pagination-size">
					<li class="page-item ${pager.startPageNo == 1 ? 'disabled' : ''}">
						<a class="page-link page-border-none text-dark" href="before?pageNo=${pager.endPageNo-pager.pagesPerGroup}" tabindex="-1" aria-disabled="true"><i class="fa-solid fa-chevron-left"></i></a>
					</li>
					<c:forEach begin="${pager.startPageNo}" end="${pager.endPageNo}" var="i">
						<li class="page-item ${pager.pageNo == i ? 'disabled' : ''}"><a class="page-link text-dark page-border-none ${pager.pageNo==i ? 'fw-bold' : ''} ${i == pager.startPageNo ? 'ms-5' : 'ms-1'} ${i == pager.endPageNo ? 'me-5' : ''}" href="${pageContext.request.contextPath}/approval/before?pageNo=${i}">${i}</a></li>
					</c:forEach>
					<li class="page-item ${pager.totalPageNo == pager.endPageNo ? 'disabled' : ''}">
						<a class="page-link page-border-none text-dark" href="before?pageNo=${pager.endPageNo+1}"><i class="fa-solid fa-chevron-right"></i></a>
					</li>
				</ul>
			</nav>
		</c:if>
...
<%@ include file="/WEB-INF/views/approval/approvalContextBeforeStep.jsp" %>
